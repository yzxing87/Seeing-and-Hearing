...
def init_prompt(prompt, pipeline):
    ...
    text_embeddings = text_embeddings.to(dtype=pipeline.text_encoder.dtype, device=pipeline.device)
    ...
    uncond_embeddings = pipeline.text_encoder(uncond_input.input_ids.to(pipeline.device))#[0]
    uncond_embeddings = uncond_embeddings.text_embeds
    uncond_embeddings = F.normalize(uncond_embeddings, dim=-1)

    uncond_embeddings = uncond_embeddings.to(dtype=pipeline.text_encoder.dtype, device=pipeline.device)
...
def get_noise_pred_single(latents, t, context, unet):
    ...
    noise_pred = unet(latents, t, encoder_hidden_states=None, class_labels=context)["sample"]
    return noise_pred
